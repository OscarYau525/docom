...
class Scheduler(object):
    ...
    def init_eval_timepoints(self, n_points=20):
        if self.conf.log_eval:
            end_it = self.conf.num_iterations
            self.eval_timepoints = sorted(list(set([int(10**( i/n_points * log10(end_it))) for i in range(n_points+1)])))
    ...
    def is_eval(self):
        if self.conf.log_eval:
            ...
            return self.local_index % self.conf.eval_freq == 0
