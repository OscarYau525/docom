...
class Fully_SPDA(Optimizer): # current implementation uses local updates
    def __init__(
        self,
        params,
        lr=required,
        ...
        super(Fully_SPDA, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(Fully_SPDA, self).__setstate__(state)
    ...
    def step(self, **kargs):
        self.n_bits = 0
        # self.gamma = self.gamma_0 / (self.it + 1)
        ...
        for nei in self.neighbors_info:
            if nei in self.sync_buffer["edge_result"]:
                ...
                dual_grad_buffer[nei].buffer[ indices ] += self.gamma * sign * (flatten_params.buffer[indices] - sparse_values)
        

        # apply dual variable to prm, i.e., sparse update under the current round mask
        for nei in self.compressor.active_neighbors:
            ...
            for layer_n, groups in enumerate(self.param_groups):
                ...
                for prm, lambdas in zip(groups["params"], groups["lambdas"]):
                    prm.view(-1).data[ selected_idx ] -= sign * self.gamma * lambdas[nei].view(-1).data[ selected_idx ].detach().clone()
        
        # apply prm.grad + weight_decay to local model
        utils.apply_gradient(
            self.param_groups, self.state, apply_grad_to_model=True, set_model_prm_as_grad=False, lr=self.gamma
        )

        #  ==== do aggregate for primal variable ==== 
        params, flatten_params = self.get_prm(self.param_groups, self.param_names)
        ...
        d_buffer.buffer += self.alpha

        for nei in self.sync_buffer["edge_result"]:
            _, indices = self.sync_buffer["edge_result"][nei]
            d_buffer.buffer[indices] += 2 * self.gamma
