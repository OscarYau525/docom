...
class Logger:
    ...
    def __init__(self, file_folder):
        ...
        self.redirect_new_json() # avoid existing jsons
    ...
    def upload_gcloud(self, bucket_path):
        """upload all existing json files to gcloud."""
        subprocess.check_call(['gsutil', 'cp', self.file_folder, bucket_path])
        print("done: {}".format(" ".join(['gsutil', 'cp', self.file_folder, bucket_path])))
...
def display_training_stat(conf, scheduler, tracker, n_bits_to_transmit, display=False):
    ...
    conf.logger.log_metric(
        ...
        values={
            ...
            "learning_rate": scheduler.old_learning_rate,
            "local_index": scheduler.local_index,
            "n_bits_to_transmit": n_bits_to_transmit / 8 / (2 ** 20), # MBtyes
            **tracker(),
        },
        tags={"split": "train"},
        display=display,
...
def display_consensus_distance(conf, scheduler, dist):
    current_time = time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    conf.logger.log_metric(
        ...
        tags={"type": "averaged_model"},
        display=True
    )
    conf.logger.save_json()

def display_custom(conf, scheduler, values, label):
    current_time = time.strftime("%Y-%m-%d %H:%M:%S")
