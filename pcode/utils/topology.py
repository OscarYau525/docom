...
class ERGraph(PhysicalLayout):
    ...
    def _define_graph(self, n_mpi_process, p=0.5):
        np.random.seed(0)
        # with lazy metropolis weights
        W = np.eye(n_mpi_process)
        for i in range(n_mpi_process):
            for j in range(i+1, n_mpi_process):
                if np.random.rand() < p:
                    W[i,j] = 1
                    W[j,i] = 1
        deg = np.sum(W, axis=1)
        assert np.all(deg > 1), "isolated nodes" # debug: does not check connectedness here
        _W = np.zeros_like(W)
        for i in range(n_mpi_process):
            for j in range(n_mpi_process):
                if i != j and W[i,j] == 1:
                    _W[i,j] = 1/(2 * max(deg[i], deg[j]))
        for i in range(n_mpi_process):
            _W[i,i] = 1 - sum(_W[i])
